...
function renderButtons() {
  ...
  document.querySelectorAll('.btn').forEach((btn) => {
    const ingredient = btn.classList[1].split('-')[1];
    if (state[ingredient]) {
...
function renderPrice() {
  ...
  const listPrice = document.querySelector('.price ul');
  ...
  for (let ingredient in ingredients) {
    if (state[ingredient]) {
      total += ingredients[ingredient].price; // Add ingredient price to total
      listItems += `<li>$${ingredients[ingredient].price} ${ingredients[ingredient].name}</li>`;
    }
  }
  
  listPrice.innerHTML = listItems;
  totalPrice.innerHTML = `$${total}`; // Properly display the total
}
